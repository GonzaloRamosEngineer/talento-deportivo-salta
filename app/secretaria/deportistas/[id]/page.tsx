  ...
  return (
    <GuardiaSecretaria>
      <div className="flex flex-col gap-5">
        ...
        {series.length > 0 ? <><section><h2 className="mb-2 text-sm font-extrabold">Evolución por métrica y protocolo</h2><div className="flex gap-2 overflow-x-auto pb-1">{series.map((serie) => <button key={serie.clave} onClick={() => setSerieActiva(serie.clave)} className={cn("shrink-0 rounded-full border px-4 py-2 text-xs font-bold", activa?.clave === serie.clave ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card")}>{serie.nombre} · {serie.protocolo}</button>)}</div></section>{activa && atributo && <section className="rounded-3xl border border-border bg-card p-5"><div><h2 className="text-base font-extrabold">{activa.nombre} <span className="text-sm text-muted-foreground">· {activa.protocolo}</span></h2><p className="text-xs text-muted-foreground">Cada protocolo conserva su propia serie; los intentos no se promedian.</p></div><div className="mt-3"><EvolutionChart serie={activa.mediciones} atributo={atributo} /></div><div className="mt-4 divide-y divide-border">{[...activa.filas].reverse().map((medicion) => <div key={medicion.id} className="flex items-center justify-between gap-3 py-3"><div><p className="text-sm font-bold">{fecha(medicion.fecha)} · intento {medicion.intento}</p><p className="text-xs text-muted-foreground">Evaluó {medicion.evaluadoPor}</p>{medicion.loteId && <Link href={`/secretaria/jornadas/${medicion.loteId}`} className="mt-1 inline-flex items-center gap-1 text-[11px] font-extrabold text-primary">Ver planilla de origen <ArrowUpRight className="size-3" /></Link>}</div><p className="text-base font-extrabold text-primary">{medicion.valor.toLocaleString("es-AR")} <span className="text-xs text-muted-foreground">{activa.unidad}</span></p></div>)}</div></section>}</> : <section className="rounded-3xl border border-border bg-card p-8 text-center"><p className="text-sm font-extrabold">Todavía no tiene mediciones</p><p className="mt-1 text-xs text-muted-foreground">La evolución aparecerá cuando se registre su primera jornada.</p></section>}
      </div>
    </GuardiaSecretaria>
  );
}
